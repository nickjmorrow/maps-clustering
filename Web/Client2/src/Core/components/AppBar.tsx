...
import { Modal, Button, PopulatedAppBar } from '@nickjmorrow/react-component-library';
...
export const AppBarInternal: React.FC<IProps> = ({ isAuthenticated, appName }) => {
	...
	const handleSuccess: any = (response: GoogleLoginResponse) => {
	...
	const googleModal = (
		<Modal isOpen={isGoogleModalOpen} onRequestClose={handleToggleGoogleModal}>
			<GoogleLogin clientId={clientId} buttonText="Login!" onSuccess={handleSuccess} onFailure={handleFailure} />
		</Modal>
	);
	...
	const rightComponents = isAuthenticated ? (
		<>
			<AppBarButton onClick={handleToggleLogoutModal}>Log Out</AppBarButton>
		</>
	) : (
		<>
	...
	return (
		<>
			<PopulatedAppBar appName={appName} styleVariant={1} rightComponents={rightComponents} />
			<AuthModal isOpen={isAuthModalOpen} isRegistering={false} handleToggleIsOpen={handleToggleAuthModal} />
...
export const AppBar = connect(mapStateToProps, null)(AppBarInternal);
